...
roles/appli/tests/inventory
roles/appli/tests/test.yml
roles/appli/vars/main.yml
```

Les dossiers `files/` et `templates/` sont créés **vides** (d'où leur absence dans un `find -type f`), et `init` ajoute un dossier `tests/` (un mini-inventaire + un playbook de test). L'arborescence exacte peut varier d'une version d'ansible-core à l'autre — ce qui compte, c'est la présence des `main.yml` par dossier.
...
The `files/` and `templates/` folders are created **empty** (hence their absence from a `find -type f`), and `init` adds a `tests/` folder (a mini-inventory + a test playbook). The exact tree may vary from one ansible-core version to another — what matters is the presence of each folder's `main.yml`.
...
:::lang fr
**✅ Vérification :** dans la sortie, la tâche `TASK [commun : Poser un marqueur...]` s'exécute **avant** les tâches de `appli` — c'est l'ordre imposé par la dépendance, sans que tu l'aies listée dans `roles:`. Le fichier `/tmp/appli-base.txt` existe. ⚠️ Attention : le `meta/main.yml` généré par `init` contient **déjà** une ligne `dependencies: []` (vide) au niveau racine, en plus du bloc `galaxy_info`. **Remplace** cette ligne vide par ton bloc `dependencies:` — n'en ajoute **pas** un second, sinon tu obtiens un avertissement `Found duplicate mapping key 'dependencies'` et un comportement fragile.
:::

:::lang en
**✅ Check:** in the output, the `TASK [commun : Poser un marqueur...]` task runs **before** `appli`'s tasks — the order imposed by the dependency, without you listing it in `roles:`. The `/tmp/appli-base.txt` file exists. ⚠️ Careful: the `meta/main.yml` generated by `init` **already** contains a (empty) `dependencies: []` line at root level, besides the `galaxy_info` block. **Replace** that empty line with your `dependencies:` block — do **not** add a second one, otherwise you get a `Found duplicate mapping key 'dependencies'` warning and fragile behavior.
:::
...
:::lang fr
**✅ Vérification :** `ansible-galaxy collection list` liste les collections **installées séparément** ; selon ton install, `ansible.builtin` peut **ne pas** y figurer (il est embarqué dans ansible-core, pas listé comme collection). Pour prouver qu'il est bien là : `ansible-doc -l ansible.builtin` affiche ses modules. Sur une install packagée (RPM/paquet `ansible`), tu verras en plus des collections préinstallées comme `community.general`, `ansible.posix`. Tu as utilisé le **FQCN** partout depuis le début de ce track (`ansible.builtin.copy`, `ansible.builtin.template`…) — c'est ça, la bonne pratique. **Sans réseau :** saute `collection install` ; `ansible.builtin` suffit pour tout ce guide. **Avec réseau :** `ansible-galaxy collection install community.general` termine par `... was installed successfully` et `ansible-doc community.general.timezone` affiche la doc du module.
:::

:::lang en
**✅ Check:** `ansible-galaxy collection list` lists **separately installed** collections; depending on your install, `ansible.builtin` may **not** appear (it's embedded in ansible-core, not listed as a collection). To prove it's there: `ansible-doc -l ansible.builtin` shows its modules. On a packaged install (RPM/`ansible` package), you'll also see preinstalled collections like `community.general`, `ansible.posix`. You've used the **FQCN** everywhere since the start of this track (`ansible.builtin.copy`, `ansible.builtin.template`…) — that's the best practice. **Without network:** skip `collection install`; `ansible.builtin` covers this whole guide. **With network:** `ansible-galaxy collection install community.general` ends with `... was installed successfully` and `ansible-doc community.general.timezone` shows the module's doc.
:::
...
**3. Dupliquer `dependencies:` dans `meta/main.yml`.** Le fichier généré contient **déjà** un `dependencies: []` au niveau racine. **Remplace-le** — si tu en ajoutes un second, tu obtiens `Found duplicate mapping key` (et si ton bloc est placé au-dessus du `[]`, la dépendance est silencieusement ignorée). Garde-le toujours au niveau racine (colonne 0), jamais sous `galaxy_info:`.
...
**3. Duplicating `dependencies:` in `meta/main.yml`.** The generated file **already** contains a root-level `dependencies: []`. **Replace it** — if you add a second one, you get `Found duplicate mapping key` (and if your block sits above the `[]`, the dependency is silently ignored). Always keep it at root level (column 0), never under `galaxy_info:`.
